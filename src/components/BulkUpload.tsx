...
import { User, Permission, School, Teacher, Mentor, MentorSchool } from '../lib/models';
...
export default function BulkUpload({ currentUser, currentPermissions }: Props) {
  ...
  const downloadTemplate = (type: UploadType) => {
    ...
    switch (type) {
      case 'schools':
        csvContent = 'name,code,address,phone,email,enrollment_count,principal_name\n';
        csvContent += 'Example School,SCH001,"123 Main St, City",555-0100,school@example.com,500,John Doe\n';
        ...
        try {
          if (uploadType === 'schools') {
            const schoolData = {
              ...
              phone: row[3],
              email: row[4],
              enrollment_count: parseInt(row[5]) || 0,
              principal_name: row[6],
            ...
            const mentor = await db.insertOne<Mentor>(Collections.MENTORS, mentorData);

            if (mentor) {
              await db.insertOne<MentorSchool>(Collections.MENTOR_SCHOOLS, {
                mentor_id: mentor.id!,
                school_id: school.id!,
