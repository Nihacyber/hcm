...
const EditSchoolModal: React.FC<EditSchoolModalProps> = ({ isOpen, onClose, school, onUpdateSchool }) => {
  ...
  const [schoolData, setSchoolData] = useState({
    ...
    mentorCount: school.mentorCount,
    managementCount: school.managementCount,
  });

  // Initialize staff arrays with existing data
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onUpdateSchool({
      ...
      teachers: teachers.map((teacher, index) => ({
        ...teacher,
        id: school.teachers && school.teachers[index] ? school.teachers[index].id : `tch-${Date.now()}-${index}`,
        schoolId: school.id,
        trainingHistory: school.teachers && school.teachers[index] ? school.teachers[index].trainingHistory : []
      })),
      mentors: mentors.map((mentor, index) => ({
        ...mentor,
        id: school.mentors && school.mentors[index] ? school.mentors[index].id : `mtr-${Date.now()}-${index}`,
        schoolId: school.id,
        assignedTeachers: school.mentors && school.mentors[index] ? school.mentors[index].assignedTeachers : []
      })),
      management: management.map((staff, index) => ({
        ...staff,
        id: school.management && school.management[index] ? school.management[index].id : `mgmt-${Date.now()}-${index}`,
        schoolId: school.id
      })),
    });
