import React, { useState, useEffect, useMemo } from 'react';
import { Mentor, School } from '../types';
import api from '../services/firebaseService';
import Card from '../components/ui/Card';
import Button from '../components/ui/Button';
import Spinner from '../components/ui/Spinner';
import { PlusIcon, SearchIcon } from '../components/ui/Icons';
import { useDebounce } from '../hooks/useDebounce';
import AddMentorModal from '../components/modals/AddMentorModal';

const Mentors: React.FC = () => {
    const [mentors, setMentors] = useState<Mentor[]>([]);
    const [schools, setSchools] = useState<School[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [isModalOpen, setIsModalOpen] = useState(false);

    const debouncedSearchTerm = useDebounce(searchTerm, 300);

    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const [mentorsData, schoolsData] = await Promise.all([
                    api.getMentors(),
                    api.getSchools()
                ]);
                setMentors(mentorsData);
                setSchools(schoolsData);
            } catch (error) {
                console.error("Failed to fetch data:", error);
            } finally {
                setLoading(false);
            }
        };
        fetchData();
    }, []);

    const handleAddMentor = async (newMentorData: Omit<Mentor, 'id' | 'assignedTeachers'>) => {
        try {
            // Simulate API delay
            await new Promise(resolve => setTimeout(resolve, 500));

            const newMentor: Mentor = {
                ...newMentorData,
                id: `mtr-${Date.now()}`,
                assignedTeachers: []
            };
            setMentors(prevMentors => [newMentor, ...prevMentors]);
            setIsModalOpen(false);
        } catch (error) {
            console.error("Failed to add mentor:", error);
        }
    };

    const getSchoolName = (schoolId: string) => {
        const school = schools.find(s => s.id === schoolId);
        return school ? school.name : 'Unknown School';
    };

    const filteredMentors = useMemo(() => {
        return mentors.filter(mentor =>
            mentor.firstName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
            mentor.lastName.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
            mentor.email.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
            mentor.specialization.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) ||
            getSchoolName(mentor.schoolId).toLowerCase().includes(debouncedSearchTerm.toLowerCase())
        );
    }, [mentors, debouncedSearchTerm, schools]);

    return (
        <>
            <Card>
                <div className="flex flex-col md:flex-row items-center justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-200">Mentor Management</h2>
                    <div className="flex items-center space-x-4 mt-4 md:mt-0">
                        <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                                <SearchIcon className="w-5 h-5 text-gray-400" />
                            </span>
                            <input
                                type="text"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="py-2 pl-10 pr-4 text-gray-700 bg-white border rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 focus:outline-none focus:ring focus:ring-opacity-40 focus:ring-blue-400"
                                placeholder="Search mentors..."
                            />
                        </div>
                        <Button onClick={() => setIsModalOpen(true)}>
                            <PlusIcon className="w-5 h-5 mr-2 -ml-1"/>
                            Add Mentor
                        </Button>
                    </div>
                </div>

                {loading ? (
                    <div className="flex justify-center items-center h-64">
                        <Spinner />
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th scope="col" className="px-6 py-3">Name</th>
                                    <th scope="col" className="px-6 py-3">Email</th>
                                    <th scope="col" className="px-6 py-3">Phone</th>
                                    <th scope="col" className="px-6 py-3">School</th>
                                    <th scope="col" className="px-6 py-3">Specialization</th>
                                    <th scope="col" className="px-6 py-3">Experience</th>
                                    <th scope="col" className="px-6 py-3">Qualifications</th>
                                    <th scope="col" className="px-6 py-3 text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredMentors.length === 0 ? (
                                    <tr>
                                        <td colSpan={8} className="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                            No mentors found
                                        </td>
                                    </tr>
                                ) : (
                                    filteredMentors.map((mentor) => (
                                        <tr key={mentor.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {mentor.firstName} {mentor.lastName}
                                            </th>
                                            <td className="px-6 py-4">{mentor.email}</td>
                                            <td className="px-6 py-4">{mentor.phone}</td>
                                            <td className="px-6 py-4">{getSchoolName(mentor.schoolId)}</td>
                                            <td className="px-6 py-4">{mentor.specialization}</td>
                                            <td className="px-6 py-4">{mentor.experience} years</td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-wrap gap-1">
                                                    {Array.isArray(mentor.qualifications) && mentor.qualifications.length > 0 ? (
                                                        mentor.qualifications.map((qual, index) => (
                                                            <span key={index} className="px-2 py-1 text-xs bg-purple-100 text-purple-800 rounded dark:bg-purple-900 dark:text-purple-300">
                                                                {qual}
                                                            </span>
                                                        ))
                                                    ) : (
                                                        <span className="px-2 py-1 text-xs text-gray-500 italic">
                                                            No qualifications
                                                        </span>
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <a href="#" className="font-medium text-primary-600 dark:text-primary-500 hover:underline mr-4">Edit</a>
                                                <a href="#" className="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</a>
                                            </td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                        </table>
                    </div>
                )}
            </Card>
            <AddMentorModal 
                isOpen={isModalOpen}
                onClose={() => setIsModalOpen(false)}
                onAddMentor={handleAddMentor}
            />
        </>
    );
};

export default Mentors;